...
export default function Tasks({ onNavigate }) {
  ...
  const careDue = useMemo(() => journalEntries.filter((entry) => entry.careEvent && entry.nextDueDate && entry.plantId && activePlants.some((plant) => plant.id === entry.plantId)).map((entry) => ({
  ...
  const visible = allTasks.filter((task) => (
    filter === "Archived"
      ? task.archived
      : !task.archived && (filter === "All" || (filter === "Completed" ? task.completed : !task.completed))
  ));
  const save = (event) => {
  ...
  return (
    <EstatePage id="tasks-title" title="Estate Tasks" description="Planning, plant-care due dates, and completed work in one living ledger." icon="vegetable" actions={<><button className="js-estate-button" type="button" onClick={() => { const result=refreshDailyTasks(); setRefreshMessage(result.added ? `${result.added} new tasks are ready.` : "Estate tasks are up to date. No new tasks are due."); }}>Refresh Tasks</button><button className="js-estate-button is-primary" type="button" onClick={() => setShowForm(true)}>Add Task</button></>}>
      ...
      <div className="js-estate-toolbar"><div className="js-task-filters" role="group" aria-label="Filter tasks">{["Active", "Completed", "All", "Archived"].map((item) => <button key={item} className={`js-estate-button${filter === item ? " is-primary" : ""}`} type="button" aria-pressed={filter === item} onClick={() => setFilter(item)}>{item}</button>)}</div><span className="js-estate-badge is-gold">{allTasks.filter((task) => !task.completed&&!task.archived).length} active</span></div>
      ...
      {visible.filter((task)=>!task.isTemplate).length ? <div className="js-task-list">{visible.filter((task)=>!task.isTemplate).map((task) => {
        ...
        return <article className={`js-estate-card${overdue ? " is-overdue" : ""}`} key={task.id}><label><input type="checkbox" checked={task.completed} disabled={task.readOnly||task.archived} onChange={(event) => updateTask(task.id, { completed:event.target.checked, completedAt:event.target.checked ? new Date().toISOString() : null })} /><span><strong>{task.title}</strong><small>{task.archived?"Archived historical task":task.dueDate ? `${overdue ? "Overdue · " : "Due · "}${new Date(`${task.dueDate}T12:00:00`).toLocaleDateString()}` : "No due date"} · {task.source || "Estate task"}</small></span></label>{task.deletedPlantName&&<p>Former plant: {task.deletedPlantName}</p>}{task.notes && <p>{task.notes}</p>}{task.readOnly ? <button type="button" className="js-estate-button" onClick={() => onNavigate?.("Journal Timeline")}>Open care history</button> : <button type="button" className="js-estate-button is-danger" onClick={() => setDeleting(task)}>Delete</button>}{deleting?.id === task.id && <div className="js-estate-confirm" role="alert"><strong>Delete this task?</strong><div className="js-estate-confirm__actions"><button className="js-estate-button" type="button" onClick={() => setDeleting(null)}>Cancel</button><button className="js-estate-button is-danger" type="button" onClick={() => { deleteTask(task.id); setDeleting(null); }}>Delete Task</button></div></div>}</article>;
