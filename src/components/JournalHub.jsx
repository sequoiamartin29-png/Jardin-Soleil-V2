...
export default function JournalHub({ onNavigate, initialView = "recent" }) {
  ...
  return <EstatePage id="journal-hub-title" title="Journal" description="The living estate record for observations, work, harvests, weather, photographs, and garden memories." icon="herb" theme="journal">
    <div className="js-journal-hub__actions"><button className="js-estate-button is-primary" type="button" onClick={() => onNavigate?.("New Journal Entry")}>New Entry</button><button className="js-estate-button" type="button" onClick={() => onNavigate?.("New Journal Entry")}>Log Today</button></div>
