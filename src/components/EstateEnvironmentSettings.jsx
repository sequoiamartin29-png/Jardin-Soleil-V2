...
export default function EstateEnvironmentSettings({onBack}){
  ...
  return <section className="js-env-settings" aria-labelledby="environment-settings-title">
    ...
    <div className="js-env-settings__panel">
      {[
        ["liveWeather","Reactive Weather","Reflect current conditions from Open-Meteo."],
        ["seasonalEffects","Seasonal Estate Changes","Show restrained blossoms, leaves, summer wildlife, and winter atmosphere."],
        ["dayNight","Day and Night Lighting","Use local sunrise, sunset, and time for estate lighting."],
        ...(customerFeatures.buddyEnabled ? [["buddy","Buddy","Let Buddy patrol the estate and share saved garden updates."]] : []),
        ["reducedMotion","Use Reduced Motion","Stop particles, drifting effects, lightning, and swaying foliage."],
        ["sound","Sound Effects","Optional ambience is off by default."],
      ].map(([key,label,description])=><label key={key}><span><strong>{label}</strong><small>{description}</small></span><input type="checkbox" checked={Boolean(settings[key])} onChange={(event)=>updateSetting(key,event.target.checked)}/></label>)}
      <label><span><strong>Wildlife Activity</strong><small>Natural schedules occasional visitors. Minimal waits longer. Off removes the wildlife layer.</small></span><select value={settings.wildlifeActivity} onChange={(event)=>updateSetting("wildlifeActivity",event.target.value)}>{["Natural","Minimal","Off"].map(item=><option key={item}>{item}</option>)}</select></label>
