...
export default function PhotoManager() {
  ...
  const plantName = (photo) => plants.find((plant) => plant.id === photo.plantId)?.name || photo.deletedPlantName || "Unassigned estate photo";
  return <EstatePage id="photo-manager-title" title="Photo Manager" description="Preserve estate memories and connect each image to a stable plant record." icon="flower" theme="journal">
    ...
    {filteredPhotos.length ? <div className="js-estate-grid" style={{marginTop:"22px"}}>{filteredPhotos.map((photo) => <article className="js-estate-card" key={photo.id}><img src={photo.url || photo.src} alt={photo.name || `Garden photograph for ${plantName(photo)}`} style={{aspectRatio:"4/3",borderRadius:"14px",objectFit:"cover",width:"100%"}} /><span className="js-estate-kicker">{plantName(photo)}{photo.plantDeleted?" · Historical record":""}</span><h2 style={{fontFamily:"Georgia,serif",color:"#52623f"}}>{photo.name || "Garden memory"}</h2><p>{new Date(photo.date || photo.createdAt).toLocaleDateString()}</p><button className="js-estate-button is-danger" type="button" onClick={() => deletePhoto(photo.id)}>Delete Photo</button></article>)}</div> : <p className="js-estate-empty">No photographs match this plant album. Choose or create a plant, then add the first estate memory.</p>}
  </EstatePage>;
}
