...
export default function Weather(){
  ...
  return <EstatePage id="weather-title" title="Estate Weather" description="Local conditions from the dashboard’s shared Open-Meteo source, displayed in the location’s reported timezone." icon="lavender" heroOverlay={<AnimatedWeather surface="header"/>} actions={<button className="js-estate-button is-primary" type="button" onClick={()=>refreshWeather({force:true})}>Refresh Weather</button>}>
    <section className="js-weather-page-summary" aria-label="Current garden atmosphere">
      <div className="js-weather-page-summary__content">
        ...
        <p>{preview?`Previewing ${settings.previewSeason||season} ${settings.previewCondition||conditionLabel.toLowerCase()}.`:"Jardin Soleil is reflecting current local conditions."}</p>
      </div>
    </section>
    ...
    <div className="js-estate-grid">{[["Conditions",conditionLabel],["Temperature",display(weather.temperatureF,"°F")],["Feels like",display(weather.apparentTemperatureF,"°F")],["Humidity",display(weather.humidity,"%")],["Precipitation",weather.precipitationType?`${display(weather.precipitation," in")} · ${weather.precipitationType} · ${display(weather.precipitationProbability,"% chance")}`:"None reported"],["Wind",`${display(weather.windSpeedMph," mph")} · gusts ${display(weather.windGustMph," mph")}`],["Cloud cover",display(weather.cloudCover,"%")],["Last updated",weather.observedAt?new Date(weather.observedAt).toLocaleString():"Unavailable"],["Status",weather.isStale?"Last known conditions":"Current cached observation"]].map(([label,value])=><article className="js-estate-card" key={label}><span className="js-estate-kicker">{label}</span><h2 style={{color:"#52623f",fontFamily:"Georgia,serif"}}>{value}</h2></article>)}</div>}
